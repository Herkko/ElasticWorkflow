...
class ActorUpdate extends Actor {

  import ActorUpdate._
  ...
  def receive = {
    ...
    case Update() => {
      Logger.info("Model was updated, send it to " + (Option(members).filterNot(_.isEmpty).map(_.size + " members").getOrElse("no one")))
...
object ActorUpdate {
  ...
  lazy val ref = system.actorOf(Props[ActorUpdate])
}