...
object Swimlane extends Controller {
    ...
    request.body.asJson.map { json =>
      {
        ...
        val elementTypeId = (json \ "elementTypeId").asOpt[Int].getOrElse(1)
        val value = (json \ "value").asOpt[String].getOrElse("Swimlane")
        val size = (json \ "size").asOpt[Int].getOrElse(0)
        val x = (json \ "cx").asOpt[String].getOrElse("100")
        val y = (json \ "cy").asOpt[String].getOrElse("100")
        println( value + " " + size + " " + x + " " + y)
